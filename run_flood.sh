...
LOG="${LOG:-reports/rf_link_flood_screenshot.jsonl}"
OUT="${OUT:-reports/rf_link_flood_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "      scenario:    scenarios/rf_link/link_flood.yaml"
...
echo "Flood rate/duration are configured in scenarios/rf_link/link_flood.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    python3 -m cfs_security_benchmark.runner.run_rf_link \
      --scenario scenarios/rf_link/link_flood.yaml \
      --target '$TARGET' \
