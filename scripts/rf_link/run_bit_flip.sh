...
LOG="${LOG:-reports/rf_link_bit_flip_screenshot.jsonl}"
OUT="${OUT:-reports/rf_link_bit_flip_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "      scenario:    scenarios/rf_link/link_bit_flip.yaml"
...
echo "Flip offset/bit are configured in scenarios/rf_link/link_bit_flip.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    ...
    python3 -m cfs_security_benchmark.runner.run_rf_link \
      --scenario scenarios/rf_link/link_bit_flip.yaml \
      --target '$TARGET' \
