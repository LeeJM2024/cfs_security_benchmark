...
namespace Nos3
{
        ...
        : GPSSimHardwareModelCommon(config)
    {
        ...
        // NAV's child task reads this USART passively. Keep the real simulator
        // on the NOS time bus so it publishes the device's normal periodic
        // telemetry stream; a request/reply-only model leaves both endpoints
        // waiting and produces permanent NAV read errors.
        _time_bus.reset(new NosEngine::Client::Bus(_hub, connection_string, time_bus_name));
        sim_logger->debug("GPSSimHardwareModelOEM615::GPSSimHardwareModelOEM615:  Time bus %s now active.",
                          time_bus_name.c_str());

        std::string bus_name = "usart_0";
        std::string uart_connection_string = connection_string;
        int node_port = 0;
        ...
        if (config.get_child_optional("simulator.hardware-model.connections")) {
            BOOST_FOREACH(const boost::property_tree::ptree::value_type &v, config.get_child("simulator.hardware-model.connections")) {
                ...
                if (v.second.get("type", "").compare("usart") == 0) {
                    ...
                    uart_connection_string = v.second.get("nos-connection-string-override", connection_string);
                    break;
                }
            }
        }
        _uart_connection.reset(new NosEngine::Uart::Uart(_hub, config.get("simulator.name", "gps"), uart_connection_string,
            bus_name));
        ...
        _time_bus->add_time_tick_callback(
            std::bind(&GPSSimHardwareModelOEM615::send_periodic_data, this, std::placeholders::_1));
        ...
        // Match the flight application's normal passive-read interface.
        _periodic_logs.insert(std::map<std::string, boost::tuple<double, double>>::value_type(
            "RANGECMPA", boost::tuple<double, double>(_absolute_start_time + 10.0, 1.0)));
        _periodic_logs.insert(std::map<std::string, boost::tuple<double, double>>::value_type(
            "BESTXYZA", boost::tuple<double, double>(_absolute_start_time + 10.0, 1.0)));
        _periodic_logs.insert(std::map<std::string, boost::tuple<double, double>>::value_type(
            "GPGGA", boost::tuple<double, double>(_absolute_start_time + 10.0, 1.0)));
    }
