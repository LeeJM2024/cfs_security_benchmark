...
def run_ruby_script(*, container: str, ruby: str) -> dict[str, Any]:
    result = subprocess.run(
        ["docker", "exec", container, "/usr/bin/ruby", "-e", ruby],
        check=False,
        text=True,
    ...
    if payload is None:
        return {
            "ok": False,
            "error": "COSMOS Ruby script did not emit JSON",
            "stdout": result.stdout,
            "stderr": result.stderr,
            "returncode": result.returncode,
        }
...
def ensure_cmd_tlm_server(*, container: str, timeout_seconds: float = 30.0) -> dict[str, Any]:
    if _cmd_tlm_server_ready(container):
    ...
    log = subprocess.run(
        ["docker", "exec", container, "sh", "-lc", "tail -80 /tmp/cmd_tlm_server.log 2>/dev/null || true"],
        check=False,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    return {
        "ok": False,
        "started": True,
        "error": "COSMOS CmdTlmServer did not open port 7777 before timeout",
        "stdout": log.stdout,
        "stderr": log.stderr,
    }


def _cmd_tlm_server_ready(container: str) -> bool:
    result = subprocess.run(
        [
            "docker",
            "exec",
            ...
            "sh",
            "-lc",
            "ss -ltn 2>/dev/null | grep -q ':7777 ' || netstat -ltn 2>/dev/null | grep -q ':7777 '",
        ],
        check=False,
        text=True,
        stdout=subprocess.PIPE,
