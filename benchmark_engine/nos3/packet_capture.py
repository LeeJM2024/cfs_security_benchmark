...
class UdpPacketObservation:
    ...
    ccsds_stream_id: int | None = None
    ccsds_sequence_count: int | None = None
...
class TcpdumpCapture:
    def __init__(
        ...
        *,
        ...
        self.diagnostic_path = observation_path.with_name(observation_path.stem + "_capture.log")
    ...
    def start(self) -> None:
        ...
        if self.image and self.mount_root:
            ...
            command = [
                ...
                "python3",
                "-m",
                "benchmark_engine.nos3.raw_capture",
                "--interface",
                self.interface,
                "--pcap",
                capture_path,
            ...
            if self.network_container:
                command.insert(5, f"--network=container:{self.network_container}")
            else:
                command.insert(5, "--net=host")
        else:
        ...
        if self.process.poll() is not None:
            self._write_diagnostics(*self.process.communicate())
            raise RuntimeError(f"packet capture exited during startup; inspect {self.diagnostic_path}")
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            self.process.kill()
            stdout, stderr = self.process.communicate(timeout=5)
        self._write_diagnostics(stdout, stderr)
        self.process = None

    def _write_diagnostics(self, stdout: str | None, stderr: str | None) -> None:
        ...
        for line in observation_path.read_text(encoding="utf-8").splitlines():
            ...
            observations.append(
                UdpPacketObservation(
                    ...
                    payload_sha256=str(item["payload_sha256"]) if item.get("payload_sha256") else None,
                    ccsds_stream_id=_optional_int(item.get("ccsds_stream_id")),
                    ccsds_sequence_count=_optional_int(item.get("ccsds_sequence_count")),
                )
            )
        return observations
...
def summarize_link_packets(
    ...
    *,
    ...
    proxy_hashes = {packet.payload_sha256 for packet in to_proxy if packet.payload_sha256}
    target_hashes = {packet.payload_sha256 for packet in to_target if packet.payload_sha256}
    return {
        ...
        "proxy_payload_hashes": sorted(proxy_hashes),
        "target_payload_hashes": sorted(target_hashes),
        "matching_payload_hashes": sorted(proxy_hashes & target_hashes),
        "proxy_ccsds_sequence_counts": [packet.ccsds_sequence_count for packet in to_proxy if packet.ccsds_sequence_count is not None],
        "target_ccsds_sequence_counts": [packet.ccsds_sequence_count for packet in to_target if packet.ccsds_sequence_count is not None],
    }
