...
BENCHMARK_MARKERS = [
    "SP-003 cFS app crash and restart pressure command interface",
    "SP003 runtime pressure app",
]
...
def main() -> None:
    ...
    print("SP003 app crash/restart pressure integration uninstalled.")
    print("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Remove SP003 cFS app crash/restart pressure benchmark artifacts from a NOS3 source tree."
    )
    parser.add_argument(
        "--nos3-root",
        type=Path,
...
def remove_build_and_runtime_artifacts(nos3_root: Path) -> None:
    paths = [
        nos3_root / "fsw/build/exe/cpu1/cf/sp003_crash.so",
        nos3_root / "fsw/build/exe/cpu1/cf/sp003_state.dat",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp003_app_crash_restart",
    ]
    ...
    if cf_dir.exists():
        for path in cf_dir.glob("sp003_*"):
            remove_path(nos3_root, path)
