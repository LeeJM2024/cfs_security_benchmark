...
BENCHMARK_MARKERS = [
    "SP-008 Subsystem state spoofing command and telemetry interface",
    "SP008 state spoof app",
]
...
def main() -> None:
    ...
    print("SP008 subsystem state spoof integration uninstalled.")
    print("")
    print("Next: run `make config` before the next normal NOS3 build/launch.")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Remove SP008 subsystem state spoof benchmark artifacts from NOS3.")
    parser.add_argument(
        "--nos3-root",
        type=Path,
        default=None,
...
def remove_build_and_runtime_artifacts(nos3_root: Path) -> None:
    paths = [
        nos3_root / "fsw/build/exe/cpu1/cf/sp008.so",
        nos3_root / "fsw/build/exe/cpu1/cf/sp008_state_spoof.so",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp008",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp008_state_spoof",
    ]
    ...
    if cf_dir.exists():
        for path in cf_dir.glob("sp008_*"):
            remove_path(nos3_root, path)
...
def ensure_under(root: Path, path: Path) -> None:
    ...
    if resolved_path != resolved_root and resolved_root not in resolved_path.parents:
        raise SystemExit(f"Refusing to remove path outside NOS3 root: {resolved_path}")
