...
BENCHMARK_MARKERS = [
    "SP-007 Resource exhaustion command and telemetry interface",
    "SP007 resource exhaustion app",
]
...
def main() -> None:
    ...
    print("SP007 resource exhaustion integration uninstalled.")
    print("")
    print("Next: run `make config` before the next normal NOS3 build/launch.")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Remove SP007 resource exhaustion benchmark artifacts from NOS3.")
    parser.add_argument(
        "--nos3-root",
        type=Path,
        default=None,
...
def remove_targets_entry(path: Path) -> None:
    if not path.exists():
        print(f"targets.cmake already absent: {path}")
        return
...
def remove_build_and_runtime_artifacts(nos3_root: Path) -> None:
    paths = [
        nos3_root / "fsw/build/exe/cpu1/cf/sp007.so",
        nos3_root / "fsw/build/exe/cpu1/cf/sp007_resource_exhaustion.so",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp007",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp007_resource_exhaustion",
    ]
    ...
    if cf_dir.exists():
        for path in cf_dir.glob("sp007_*"):
            remove_path(nos3_root, path)
