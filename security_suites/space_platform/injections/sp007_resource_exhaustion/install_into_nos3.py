...
COMPONENT_PATH = "sp007_resource_exhaustion/fsw/cfs"
APP_ENTRY = f"        {COMPONENT_PATH}\n"
COMPONENT_NAME = "sp007_resource_exhaustion"
COSMOS_DICT_NAME = "SP007_RESOURCE_EXHAUSTION.txt"
VERIFY_SCRIPT_NAME = "sp007_verify.rb"
DEFAULT_OPERATOR_CONTAINER = "cosmos-openc3-operator-1"
DEFAULT_NOS3_ROOT = Path("/home/leejm/nos3")
STARTUP_ENTRY = "CFE_APP, sp007,                      SP007_AppMain,            SP007,            77, 32768, 0x0, 0;\n"
...
def main() -> None:
    ...
    print("SP007 resource exhaustion integration installed.")
    print("")
    print("Benchmark workflow:")
    print("  1. install once: copy SP007 FSW app, COSMOS dictionary, and verifier")
    print("  2. build once:   run `make config && make fsw` from NOS3 root")
    print("  3. run/verify:   launch NOS3, then run one profile or the full verifier")
    print("  4. uninstall:    run uninstall_from_nos3.py after testing is complete")
    print("")
    print("Single-profile verifier examples:")
    for profile in ["CPU_BUSY_STARVATION", "SB_TELEMETRY_FLOOD", "EVS_EVENT_STORM", "CF_STORAGE_FILL", "OSAL_QUEUE_EXHAUST"]:
        print(f"  docker exec -e SP007_ONLY={profile} {args.operator_container} /usr/bin/ruby /tmp/{VERIFY_SCRIPT_NAME}")
    print("")
    print("Full verifier:")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install SP007 resource exhaustion benchmark artifacts into NOS3.")
    parser.add_argument(
        "--nos3-root",
        type=Path,
        default=None,
...
def require_file(path: Path) -> None:
    if not path.exists():
        raise SystemExit(f"Missing required SP007 artifact: {path}")
...
def remove_startup_component(text: str) -> str:
    return "".join(
        ...
        if "sp007_resource_exhaustion" not in line and "CFE_APP, sp007," not in line and "SP007_AppMain" not in line
    )
...
def backup_once(path: Path) -> None:
    ...
    backup = path.with_suffix(path.suffix + ".sp007.bak")
