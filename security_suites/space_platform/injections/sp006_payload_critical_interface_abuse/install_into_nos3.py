...
COMPONENT_PATH = "sp006_payload_abuse/fsw/cfs"
APP_ENTRY = f"        {COMPONENT_PATH}\n"
COMPONENT_NAME = "sp006_payload_abuse"
COSMOS_DICT_NAME = "SP006_PAYLOAD_ABUSE.txt"
VERIFY_SCRIPT_NAME = "sp006_verify.rb"
DEFAULT_OPERATOR_CONTAINER = "cosmos-openc3-operator-1"
DEFAULT_NOS3_ROOT = Path("/home/leejm/nos3")
STARTUP_ENTRY = "CFE_APP, sp006,                      SP006_AppMain,            SP006,            76, 32768, 0x0, 0;\n"
...
def main() -> None:
    ...
    enable_required_components(nos3_root)
    ...
    print("SP006 payload critical interface abuse integration installed.")
    print("")
    print("Benchmark workflow:")
    print("  1. prepare/install: copy SP006 FSW app, COSMOS dictionary, and verifier")
    print("  2. build once:      run `make config && make fsw` from NOS3 root")
    print("  3. run/verify:      launch NOS3, then run the installed SP006 verifier")
    ...
    print("")
    print("Single-profile verifier examples:")
    print(f"  docker exec -e SP006_ONLY=CAM_HWLIB_OUT_OF_ORDER {args.operator_container} /usr/bin/ruby /tmp/{VERIFY_SCRIPT_NAME}")
    print(f"  docker exec -e SP006_ONLY=RW_DISABLED_SET_TORQUE {args.operator_container} /usr/bin/ruby /tmp/{VERIFY_SCRIPT_NAME}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install SP006 payload critical interface abuse benchmark artifacts into NOS3."
    )
    parser.add_argument(
        "--nos3-root",
        type=Path,
...
def require_file(path: Path) -> None:
    if not path.exists():
        raise SystemExit(f"Missing required SP006 artifact: {path}")


def enable_required_components(nos3_root: Path) -> None:
    sc_config = nos3_root / "cfg/spacecraft/sc-mission-config.xml"
    backup_once(sc_config)
    text = sc_config.read_text(encoding="utf-8")
    replacements = [
        ("<cam>\n\t\t\t<enable>false</enable>\n\t\t</cam>", "<cam>\n\t\t\t<enable>true</enable>\n\t\t</cam>"),
        ("<cam>\n            <enable>false</enable>\n        </cam>", "<cam>\n            <enable>true</enable>\n        </cam>"),
    ]
    for old, new in replacements:
        if old in text:
            sc_config.write_text(text.replace(old, new, 1), encoding="utf-8")
            print(f"Enabled CAM/arducam component for SP006 testing: {sc_config}")
            return
    print(f"CAM component already enabled or XML layout changed: {sc_config}")
...
def remove_startup_component(text: str) -> str:
    return "".join(line for line in text.splitlines(keepends=True) if "sp006_payload_abuse" not in line and "SP006_AppMain" not in line)
...
def backup_once(path: Path) -> None:
    ...
    backup = path.with_suffix(path.suffix + ".sp006.bak")
