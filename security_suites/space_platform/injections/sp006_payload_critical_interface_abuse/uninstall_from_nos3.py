...
BENCHMARK_MARKERS = [
    "SP-006 Payload critical interface abuse command and telemetry interface",
    "SP006 payload abuse app",
]
...
def main() -> None:
    ...
    restore_required_components(nos3_root)

    print("SP006 payload critical interface abuse integration uninstalled.")
    print("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Remove SP006 payload critical interface abuse benchmark artifacts from NOS3."
    )
    parser.add_argument(
        "--nos3-root",
        type=Path,
...
def remove_startup_entry(path: Path) -> None:
    ...
    text = path.read_text(encoding="utf-8")
    updated = "".join(line for line in text.splitlines(keepends=True) if "sp006_payload_abuse" not in line and "CFE_APP, sp006," not in line and "SP006_AppMain" not in line)
    if updated == text:
...
def restore_required_components(nos3_root: Path) -> None:
    sc_config = nos3_root / "cfg/spacecraft/sc-mission-config.xml"
    backup = sc_config.with_suffix(sc_config.suffix + ".sp006.bak")
    if backup.exists():
        ensure_under(nos3_root, backup)
        shutil.copy2(backup, sc_config)
        backup.unlink()
        print(f"Restored spacecraft component configuration: {sc_config}")
    else:
        print(f"No SP006 spacecraft component backup found: {sc_config}")
...
def remove_verifier(nos3_root: Path) -> None:
    ...
    for path in candidates:
        if not path.exists():
            print(f"Verifier already absent: {path}")
            continue
        ...
        if backup.exists():
            ...
            shutil.copy2(backup, path)
            backup.unlink()
            print(f"Restored previous verifier backup: {path}")
        else:
            remove_path(nos3_root, path)
...
def remove_build_and_runtime_artifacts(nos3_root: Path) -> None:
    paths = [
        nos3_root / "fsw/build/exe/cpu1/cf/sp006.so",
        nos3_root / "fsw/build/exe/cpu1/cf/sp006_payload_abuse.so",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp006",
        nos3_root / "fsw/build/amd64-nos3/default_cpu1/apps/sp006_payload_abuse",
    ]
    ...
    if cf_dir.exists():
        for path in cf_dir.glob("sp006_*"):
            remove_path(nos3_root, path)
