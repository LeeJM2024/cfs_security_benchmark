...
LOG="${LOG:-artifacts/reports/rf_link_reorder_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_reorder_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "      scenario:    security_suites/rf_link/scenarios/link_reorder.yaml"
...
echo "Reorder window size is configured in security_suites/rf_link/scenarios/link_reorder.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    ...
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_reorder.yaml \
      --target '$TARGET' \
