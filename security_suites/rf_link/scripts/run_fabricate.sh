...
LOG="${LOG:-artifacts/reports/rf_link_fabricate_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_fabricate_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "      target:      $TARGET"
echo "      scenario:    security_suites/rf_link/scenarios/link_fabricate.yaml"
...
echo "Payload/count/interval are configured in security_suites/rf_link/scenarios/link_fabricate.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_fabricate.yaml \
      --target '$TARGET' \
