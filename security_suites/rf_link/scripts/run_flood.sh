...
CONTAINER="${CONTAINER:-cfs-benchmark-rf-link-flood}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-cosmos-openc3-operator-1}"
LOG="${LOG:-artifacts/reports/rf_link_flood_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_flood_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "[3/5] Starting RF-LINK-006 command flood benchmark"
echo "      link:        $LINK"
...
echo "      scenario:    security_suites/rf_link/scenarios/link_flood.yaml"
echo "      duration:    ${DURATION}s"
echo "      log:         $LOG"
echo
echo "This scenario actively sends random UDP packets to the target."
echo "You do not need to send CFS_RADIO CFE_ES_NOOP for flood."
echo "Flood rate/duration are configured in security_suites/rf_link/scenarios/link_flood.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_flood.yaml \
      --target '$TARGET' \
...
echo "OK: no DNAT rule is used for flood mode"
...
if path.exists():
    for line in path.read_text().splitlines():
        ...
        if event == "packet_flooded":
            destination = str(item.get("destination", "unknown"))
            destinations[destination] = destinations.get(destination, 0) + 1

flooded = counts.get("packet_flooded", 0)

print("log:", path)
print("counts:", counts)
print("flood_destinations:", destinations)

if flooded > 0:
    print("RESULT: SUCCESS - flood packets were generated and sent.")
else:
    print("RESULT: NO FLOOD OBSERVED - inspect scenario type and target reachability.")
PY
