...
CONTAINER="${CONTAINER:-cfs-benchmark-rf-link-bit-flip}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-cosmos-openc3-operator-1}"
LOG="${LOG:-artifacts/reports/rf_link_bit_flip_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_bit_flip_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "[3/5] Starting RF-LINK-005 bit flip benchmark"
echo "      link:        $LINK"
...
echo "      scenario:    security_suites/rf_link/scenarios/link_bit_flip.yaml"
...
echo "Flip offset/bit are configured in security_suites/rf_link/scenarios/link_bit_flip.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    ...
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_bit_flip.yaml \
      --target '$TARGET' \
...
if path.exists():
    for line in path.read_text().splitlines():
        ...
        if event == "packet_bit_flipped":
            flip_offsets.append(item.get("offset"))
            flip_bits.append(item.get("bit"))

received = counts.get("packet_received", 0)
flipped = counts.get("packet_bit_flipped", 0)
forwarded = counts.get("packet_forwarded", 0)

print("log:", path)
print("counts:", counts)
if flip_offsets:
    print("flip_offsets:", flip_offsets)
    print("flip_bits:", flip_bits)

if received > 0 and flipped > 0 and forwarded > 0:
    print("RESULT: SUCCESS - packets were captured, mutated, and forwarded.")
elif received > 0 and flipped == 0:
    print("RESULT: CAPTURED BUT NOT MUTATED - inspect scenario direction/type.")
else:
