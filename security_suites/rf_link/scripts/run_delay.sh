...
CONTAINER="${CONTAINER:-cfs-benchmark-rf-link-delay}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-cosmos-openc3-operator-1}"
LOG="${LOG:-artifacts/reports/rf_link_delay_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_delay_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "[3/5] Starting RF-LINK-003 command delay benchmark"
echo "      link:        $LINK"
...
echo "      scenario:    security_suites/rf_link/scenarios/link_delay.yaml"
...
echo "Delay is configured in security_suites/rf_link/scenarios/link_delay.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    ...
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_delay.yaml \
      --target '$TARGET' \
...
if path.exists():
    for line in path.read_text().splitlines():
        ...
        if event == "packet_received":
            received_times.append(datetime.fromisoformat(item["ts"]))
        elif event == "packet_forwarded":
            forwarded_times.append(datetime.fromisoformat(item["ts"]))

latencies = [
    (forwarded - received).total_seconds()
    for received, forwarded in zip(received_times, forwarded_times)
]

print("log:", path)
print("counts:", counts)
if latencies:
    print("observed_forward_delays_seconds:", [round(value, 3) for value in latencies])
    print("max_forward_delay_seconds:", round(max(latencies), 3))
else:
    print("observed_forward_delays_seconds:", [])

if latencies and max(latencies) >= 1.5:
    print("RESULT: SUCCESS - packets were delayed before forwarding.")
elif counts.get("packet_received", 0) > 0:
    print("RESULT: CAPTURED BUT DELAY NOT OBSERVED - inspect timestamps or scenario delay_ms.")
else:
