...
CONTAINER="${CONTAINER:-cfs-benchmark-rf-link-drop}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-cosmos-openc3-operator-1}"
LOG="${LOG:-artifacts/reports/rf_link_drop_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_drop_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "[3/5] Starting RF-LINK-002 packet drop benchmark"
echo "      link:        $LINK"
...
echo "      scenario:    security_suites/rf_link/scenarios/link_drop.yaml"
echo "      duration:    ${DURATION}s"
echo "      log:         $LOG"
echo
echo "For LINK=radio, send CFS_RADIO CFE_ES_NOOP repeatedly after socket_bound appears."
echo "For LINK=debug, send CFS CFE_ES_NOOP repeatedly after socket_bound appears."
echo "Drop probability is configured in security_suites/rf_link/scenarios/link_drop.yaml."
echo
...
docker run --rm --name "$CONTAINER" \
  ...
  sh -lc "
    ...
    python3 -m benchmark_engine.rf_link.simulator_runner \
      --scenario security_suites/rf_link/scenarios/link_drop.yaml \
      --target '$TARGET' \
...
dropped = max(received - forwarded, 0)

print("log:", path)
print("counts:", counts)
print("inferred_dropped:", dropped)

if received > 0 and dropped > 0:
    print("RESULT: SUCCESS - packets were captured and at least one was dropped.")
elif received > 0:
    print("RESULT: NO DROP OBSERVED - run again or send more NOOP commands.")
else:
