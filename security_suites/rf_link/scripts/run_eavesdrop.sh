...
CONTAINER="${CONTAINER:-cfs-benchmark-rf-link}"
COSMOS_CONTAINER="${COSMOS_CONTAINER:-cosmos-openc3-operator-1}"
LOG="${LOG:-artifacts/reports/rf_link_eavesdrop_screenshot.jsonl}"
OUT="${OUT:-artifacts/reports/rf_link_eavesdrop_screenshot.out}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
COSMOS_PID=""
...
echo "[3/5] Starting RF-LINK-001 eavesdrop benchmark"
echo "      link:        $LINK"
...
if received > 0 and forwarded > 0:
    print("RESULT: SUCCESS - packets were captured and forwarded.")
else:
